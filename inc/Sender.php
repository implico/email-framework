...
class Sender
{
	...
	public function __construct($config, $script, $toAddress, $toAddressFilename, $fromName, $fromAddress, $subject, $useMinified, $interval, $stopOnError)
	{
		...
		if (!$toAddress)
			$toAddress = $this->smarty->getConfigVars('mSenderDefaultToAddress');
		...
		if (!$fromName)
			$fromName = $this->smarty->getConfigVars('mSenderFromName');
		$this->fromName = $fromName;
		
		if (!$fromAddress)
			$fromAddress = $this->smarty->getConfigVars('mSenderFromAddress');
		
		$this->fromAddress = $fromAddress;
		
		if (!$subject)
			$subject = $this->smarty->getConfigVars('mSenderSubject');
		$this->subject = $subject;
		
		if ($useMinified === null)
			$useMinified = $this->smarty->getConfigVars('mSenderUseMinified');
	...
	{
		$smarty = new \Smarty();
		$smarty->setCompileDir(IE_COMPILE_DIR);
		...
		if (file_exists(IE_PROJECTS_DIR.'config.conf'))
			$smarty->configLoad(IE_PROJECTS_DIR.'config.conf');
	...
	protected function configureMailer($content)
	{
		...
		$mail->Host = $this->smarty->getConfigVars('mSenderHost');
		
		$mail->Username = $this->smarty->getConfigVars('mSenderUser');
		$mail->Password = $this->smarty->getConfigVars('mSenderPassword');
		
		$secure = $this->smarty->getConfigVars('mSenderSecure');
		if ($secure)
			$mail->SMTPSecure = $secure;
		
		$port = $this->smarty->getConfigVars('mSenderPort');
		if ($port) {
			$mail->Port = $port;
		}
		...
		if ($randomUrl = $this->smarty->getConfigVars('mSenderGenerateRandomUnsubscribe'))
	...
	public function run($callback = null)
	{
		...
		if ($content = @file_get_contents($file)) {
			...
			foreach ($this->toAddress as $toAddress) {
				...
				if ($this->validateEmail($toAddress)) {
