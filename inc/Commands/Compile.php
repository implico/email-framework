...
class Compile extends Command
{
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		//Smarty core config file load
		$smarty->configLoad(IE_CORE_DIR.'config.conf');
		
		//optional master config file
		...
		$formatter = null;
		if ($outputFormatted)
			$formatter = new \Gajus\Dindent\Indenter(array('indentation_character' => "\t"));
		...
		while (true) {
			...
			if (!$watch || ($compileDirStamp != $this->getDirStamp($checkDirs))) {
				...
				if (file_exists($configFile)) {
					...
					$smarty->assign('configFile', $configFile);
				}
				
				//set encoding
				$outputEncoding = $smarty->getConfigVars('mEncoding');
				...
				foreach ($scripts as $i => $script) {
					...
					if (file_exists($configFileScript)) {
						...
						$smarty->assign('configFileScript',  $configFileScript);
					}
	...
	/**
	 * Calculate direcotry(ies) file stamp
	 * 
